// Clouds Background (WebGPU WGSL) - procedural drifting clouds.
...
fn noise(p: vec2<f32>) -> f32 {
    ...
    let h = max(vec3<f32>(0.5, 0.5, 0.5) - vec3<f32>(dot(a,a), dot(b,b), dot(c,c)), vec3<f32>(0.0, 0.0, 0.0));
    let h4 = h * h * h * h;

    let n = h4 * vec3<f32>(
        dot(a, hash2(i + vec2<f32>(0.0, 0.0))),
        dot(b, hash2(i + o)),
        dot(c, hash2(i + vec2<f32>(1.0, 1.0)))
    );
...
var uv = (basePos * 0.0016) * (shaderParams.scale * 1.1) + vec2<f32>(shaderParams.seed, shaderParams.seed);
uv = uv + vec2<f32>(0.0, bob);
    // No internal advection/evolution (prevents unintended vertical drift)
...
let q = fbm(uv * 0.5 - timeVec);
    ...
    let sky = mix(shaderParams.skyTop, shaderParams.skyBottom, p.y);
    let cloudCol = vec3<f32>(1.1, 1.1, 0.95) * clamp(0.55 + 0.45 * c, 0.0, 1.0);

    let result = mix(sky, clamp(shaderParams.skyTint * sky + cloudCol, vec3<f32>(0.0,0.0,0.0), vec3<f32>(1.0,1.0,1.0)), cloud);

    let mask = mix(1.0, 1.0 - front.a, clamp(shaderParams.mask, 0.0, 1.0));
    let outA = cloud * clamp(shaderParams.opacity, 0.0, 1.0) * mask;

    let outRgb = mix(base.rgb, result, outA);

    var output: FragmentOutput;
    output.color = vec4<f32>(outRgb, max(base.a, outA));
    return output;
}
