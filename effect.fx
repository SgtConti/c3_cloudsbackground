// Clouds Background (WebGL GLSL ES 1.0) - procedural, based on Shadertoy-style fbm clouds.
// Scrolls with layout/camera by using layout-space coordinates.

// Prefer high precision when available (Linux/Mesa often treats mediump as 16-bit in fragment shaders)
...
float noise(vec2 p) {
    // Simplex-ish noise (same structure as the reference)
...
void main(void)
{
    ...
    vec2 layoutPos = mix(layoutStart, layoutEnd, nrm); // layout pixels

    // View-space normalized coords for vertical sky gradient
vec2 p = nrm;

// Layout-space (pixels) drives cloud coordinates, so clouds move with layer scrolling.
// Reduce coordinate magnitude to avoid precision loss on platforms where fragment precision is limited.
vec2 wind = vec2(uSpeedX, uSpeedY) * seconds;
float bob = sin(seconds * 0.35 + uSeed * 3.1) * (0.0025 * clamp(uBob, 0.0, 1.0));

// Convert layout pixels to a stable UV scale (tweak constant for feature size)
vec2 basePos = layoutPos + wind;

// Wrap extremely large layout coordinates to keep numbers small and stable.
// This prevents the hash/noise from collapsing when mediump precision is used.
const float PERIOD = 4096.0;
basePos = basePos - floor(basePos / PERIOD) * PERIOD;
vec2 uv = (basePos * 0.0016) * (uScale * 1.1) + vec2(uSeed, uSeed);
uv.y += bob; // subtle vertical bob in UV space
    // Drift amount (prevents unintended vertical drift)
...
vec2 timeVec = vec2(time, 0.0);
    ...
    // Composite over scene/background
